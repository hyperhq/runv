...
func (vm *Vm) StopContainer(container string) error {
	return vm.GenericOperation("StopContainer", func(ctx *VmContext, result chan<- error) {
		ctx.stopCmd(container, result)
	}, StateRunning)
}
