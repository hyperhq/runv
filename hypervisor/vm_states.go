...
func (ctx *VmContext) prepareDevice(cmd *RunPodCommand) bool {
	...
	for _, acmd := range pendings {
		...
		if idx >= 0 {
			glog.Infof("attach pending for %s", acmd.Container)
...
func (ctx *VmContext) setWindowSize(container, execId string, size *WindowSize) {
	var session uint64
	if execId != "" {
		exec, ok := ctx.vmExec[execId]
		if !ok {
			glog.Infof("can not find exec %s", execId)
			return
		}

		session = exec.Process.Stdio
	} else if container != "" {
		idx := ctx.Lookup(container)
		if idx < 0 {
			glog.Infof("can not find container %s", container)
			return
		}

		if idx > len(ctx.vmSpec.Containers) || ctx.vmSpec.Containers[idx].Process.Stdio == 0 {
			glog.Infof("tty is not configured for %s", container)
			return
		}

		session = ctx.vmSpec.Containers[idx].Process.Stdio
	} else {
		glog.Errorf("both container and exec is nil")
		return
	}

	if !ctx.ptys.isTty(session) {
		glog.Infof("the session %v is not a tty, doesn't support resize.", session)
		return
	}
...
func (ctx *VmContext) attachCmd(cmd *AttachCommand, result chan<- error) {
	...
	if cmd.Container != "" && idx < 0 {
		...
		glog.V(1).Infof("attachment is pending")
		result <- nil
		return
...
func stateInit(ctx *VmContext, ev VmEvent) {
	if processed := commonStateHandler(ctx, ev, false); processed {
		...
		case COMMAND_WINDOWSIZE:
			...
			ctx.setWindowSize(cmd.Container, cmd.ExecId, cmd.Size)
