...
function teardown() {
    rm -f $BATS_TMPDIR/runv-update-integration-test.json
...
function setup() {
    ...
    # Add cgroup path
    sed -i 's/\("linux": {\)/\1\n    "cgroupsPath": "\/runv-update-integration-test",/'  ${BUSYBOX_BUNDLE}/config.json
...
@test "update" {
  skip "runv doesn't support subcommand update yet"
    ...
    runv run -d --console-socket $CONSOLE_SOCKET test_update
    [ "$status" -eq 0 ]
    ...
    for g in MEMORY CPUSET CPU BLKIO PIDS; do
        ...
        eval CGROUP_${g}="${base_path}/runv-update-integration-test"
    done
    ...
    # update blkio-weight
    runv update test_update --blkio-weight 500
    [ "$status" -eq 0 ]
    check_cgroup_value $CGROUP_BLKIO "blkio.weight" 500

    # update cpu-period
    runv update test_update --cpu-period 900000
    [ "$status" -eq 0 ]
    check_cgroup_value $CGROUP_CPU "cpu.cfs_period_us" 900000

    # update cpu-quota
    runv update test_update --cpu-quota 600000
    [ "$status" -eq 0 ]
    check_cgroup_value $CGROUP_CPU "cpu.cfs_quota_us" 600000

    # update cpu-shares
    runv update test_update --cpu-share 200
    [ "$status" -eq 0 ]
    ...
    if [ -f "$CGROUP_MEMORY/memory.memsw.limit_in_bytes" ]; then
        ...
        runv update test_update --memory-swap -1
        [ "$status" -eq 0 ]
...
EOF
    ...
    echo $DATA > $BATS_TMPDIR/runv-update-integration-test.json

    runv update  -r $BATS_TMPDIR/runv-update-integration-test.json test_update
    [ "$status" -eq 0 ]
    check_cgroup_value $CGROUP_BLKIO "blkio.weight" 1000
    check_cgroup_value $CGROUP_CPU "cpu.cfs_period_us" 1000000
    check_cgroup_value $CGROUP_CPU "cpu.cfs_quota_us" 500000
...
@test "update rt period and runtime" {
  skip "runv doesn't support subcommand update yet"
    ...
    runv run -d --console-socket $CONSOLE_SOCKET test_update_rt
    [ "$status" -eq 0 ]

    # get the cgroup paths
    eval CGROUP_CPU="${CGROUP_CPU_BASE_PATH}/runv-update-integration-test"

    runv update  -r - test_update_rt <<EOF
{
  "cpu": {
...
EOF
    ...
    runv update test_update_rt --cpu-rt-period 900001 --cpu-rt-runtime 600001
