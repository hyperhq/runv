...
RECVTTY="${INTEGRATION_ROOT}/../recvtty/recvtty"
GOPATH="$(mktemp -d --tmpdir runv-integration-gopath.XXXXXX)"
...
# Root state path.
ROOT=$(mktemp -d "$BATS_TMPDIR/runv.XXXXXX")
...
function wait_for_container() {
	...
	echo "runv state failed to return state $statecheck $attempts times. Output: $output"
	false
}
...
function teardown_running_container() {
	...
	if [[ "${output}" == *"$1 "* ]]; then
		runv kill $1 KILL
		retry 10 1 eval "__runv state '$1' | grep -q 'stopped'"
		runv delete $1
	fi
}
...
function teardown_running_container_inroot() {
	...
	# e.g. "test_busybox" and "test_busybox_update"
	if [[ "${output}" == *"$1 "* ]]; then
		ROOT=$2 runv kill $1 KILL
		retry 10 1 eval "ROOT='$2' __runv state '$1' | grep -q 'stopped'"
		ROOT=$2 runv delete $1
	fi
}
